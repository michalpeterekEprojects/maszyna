...
bool TTrain::is_eztoer() const
{
	...
	return ((mvControlled->TrainType == dt_EZT) && (mvOccupied->BrakeSubsystem == TBrakeSubSystem::ss_ESt) && (mvControlled->Battery == true) && (mvControlled->EpFuse == true) &&
...
void TTrain::zero_charging_train_brake()
{
	...
	if ((mvOccupied->BrakeCtrlPos == -1) && (DynamicObject->Controller != AIdriver) && (Global.iFeedbackMode < 3) &&
	    ((mvOccupied->BrakeHandle == TBrakeHandle::FVel6) || (mvOccupied->BrakeHandle == TBrakeHandle::MHZ_EN57)))
	{
...
void TTrain::OnCommand_mastercontrollerincrease(TTrain *Train, command_data const &Command)
{
	...
	{
		// on press or hold
		if ((Train->ggJointCtrl.SubModel != nullptr) && (Train->mvControlled->LocalBrakePosA > 0.0))
		{
...
void TTrain::OnCommand_linebreakerclose(TTrain *Train, command_data const &Command)
{
	...
	else if (Command.action == GLFW_RELEASE)
	{
		...
		if (Train->m_linebreakerstate == 2)
		{
			...
			if ((Train->mvControlled->EngineType != TEngineType::DieselEngine) && (Train->mvControlled->EngineType != TEngineType::DieselElectric))
			{
...
void TTrain::OnCommand_headlighttoggleleft(TTrain *Train, command_data const &Command)
{
	...
	int const vehicleside = (Train->mvOccupied->ActiveCab == 1 ? end::front : end::rear);
...
void TTrain::OnCommand_headlighttogglerearleft(TTrain *Train, command_data const &Command)
{
	...
	int const vehicleside = (Train->mvOccupied->ActiveCab == 1 ? end::rear : end::front);
...
void TTrain::OnCommand_redmarkerstoggle(TTrain *Train, command_data const &Command)
{
	...
	if ((true == FreeFlyModeFlag) && (Command.action == GLFW_PRESS))
	{
		...
		int const CouplNr{
		    clamp(vehicle->DirectionGet() * (Math3D::LengthSquared3(vehicle->HeadPosition() - Global.pCamera.Pos) > Math3D::LengthSquared3(vehicle->RearPosition() - Global.pCamera.Pos) ? 1 : -1), 0,
		          1)}; // z [-1,1] zrobić [0,1]
...
void TTrain::OnCommand_instrumentlightenable(TTrain *Train, command_data const &Command)
{
	...
	if (Command.action == GLFW_PRESS)
	{
		...
		__raise Train->OnInstrumentlightChanged(0x01);
	}
}
...
void TTrain::OnCommand_instrumentlightdisable(TTrain *Train, command_data const &Command)
{
	...
	if (Command.action == GLFW_PRESS)
	{
		...
		__raise Train->OnInstrumentlightChanged(0x00);
	}
}
...
void TTrain::OnCommand_doortoggleleft(TTrain *Train, command_data const &Command)
{
	...
	if (Command.action == GLFW_PRESS)
	{
		...
		if (false == (Train->mvOccupied->ActiveCab == 1 ? Train->mvOccupied->Doors.instances[side::left].is_opening || Train->mvOccupied->Doors.instances[side::left].is_open :
	...
	else if (Command.action == GLFW_RELEASE)
	{
		...
		if (true == (Train->mvOccupied->ActiveCab == 1 ? Train->mvOccupied->Doors.instances[side::left].is_opening || Train->mvOccupied->Doors.instances[side::left].is_open :
		                                                 Train->mvOccupied->Doors.instances[side::right].is_opening || Train->mvOccupied->Doors.instances[side::right].is_open))
		{
			// open
...
void TTrain::OnCommand_doorpermitleft(TTrain *Train, command_data const &Command)
{
	...
	if (Command.action == GLFW_PRESS)
	{
		...
		Train->mvOccupied->PermitDoors((Train->mvOccupied->ActiveCab == 1 ? side::left : side::right));
...
void TTrain::OnCommand_doorpermitright(TTrain *Train, command_data const &Command)
{
	...
	if (Command.action == GLFW_PRESS)
	{
		...
		Train->mvOccupied->PermitDoors((Train->mvOccupied->ActiveCab == 1 ? side::right : side::left));
...
void TTrain::OnCommand_dooropenleft(TTrain *Train, command_data const &Command)
{
	...
	if (Command.action == GLFW_PRESS)
	{
		Train->mvOccupied->OperateDoors((Train->mvOccupied->ActiveCab == 1 ? side::left : side::right), true);
		// visual feedback
...
void TTrain::OnCommand_doorcloseleft(TTrain *Train, command_data const &Command)
{
	...
	if (Command.action == GLFW_PRESS)
	{
		...
		else
		{
			...
			Train->mvOccupied->OperateDoors((Train->mvOccupied->ActiveCab == 1 ? side::left : side::right), false);
		}
		// visual feedback
...
void TTrain::OnCommand_doortoggleright(TTrain *Train, command_data const &Command)
{
	...
	{
		// NOTE: test how the door state check works with consists where the occupied vehicle doesn't have opening doors
		if (false == (Train->mvOccupied->ActiveCab == 1 ? Train->mvOccupied->Doors.instances[side::right].is_opening || Train->mvOccupied->Doors.instances[side::right].is_open :
	...
	else if (Command.action == GLFW_RELEASE)
	{
		...
		if (true == (Train->mvOccupied->ActiveCab == 1 ? Train->mvOccupied->Doors.instances[side::right].is_opening || Train->mvOccupied->Doors.instances[side::right].is_open :
		                                                 Train->mvOccupied->Doors.instances[side::left].is_opening || Train->mvOccupied->Doors.instances[side::left].is_open))
		{
			// open
			if ((Train->mvOccupied->Doors.has_autowarning) && (Train->mvOccupied->DepartureSignal))
...
void TTrain::OnCommand_dooropenright(TTrain *Train, command_data const &Command)
{
	...
	if (Command.action == GLFW_PRESS)
	{
		...
		Train->mvOccupied->OperateDoors((Train->mvOccupied->ActiveCab == 1 ? side::right : side::left), true);
		// visual feedback
...
void TTrain::OnCommand_doorcloseright(TTrain *Train, command_data const &Command)
{
	...
	if (Command.action == GLFW_PRESS)
	{
		...
		{
			Train->mvOccupied->OperateDoors((Train->mvOccupied->ActiveCab == 1 ? side::right : side::left), false);
		}
		// visual feedback
	...
	if (Command.action == GLFW_PRESS)
	{
		if (false == Train->CabChange(1))
		{
			if (TestFlag(Train->DynamicObject->MoverParameters->Couplers[end::front].CouplingFlag, coupling::gangway))
			{
				...
				dynobj->MoverParameters->ActiveCab = (Train->DynamicObject->MoverParameters->Neighbours[end::front].vehicle_end ? -1 : 1);
	...
	if (Command.action == GLFW_PRESS)
	{
		if (false == Train->CabChange(-1))
		{
			if (TestFlag(Train->DynamicObject->MoverParameters->Couplers[end::rear].CouplingFlag, coupling::gangway))
			{
				...
				dynobj->MoverParameters->ActiveCab = (Train->DynamicObject->MoverParameters->Neighbours[end::rear].vehicle_end ? -1 : 1);
				Train->MoveToVehicle(dynobj);
			}
		}
	}
...
void TTrain::UpdateCab()
{
	    ...
	    && (DynamicObject->Mechanik->AIControllFlag))
	{
		...
		if (iCabn != ( // numer kabiny (-1: kabina B)
		                 DynamicObject->MoverParameters->ActiveCab == -1 ? 2 : DynamicObject->MoverParameters->ActiveCab))
		{
...
bool TTrain::Update(double const Deltatime)
{
	...
	if ((ggMainButton.GetDesiredValue() > 0.95) || (ggMainOnButton.GetDesiredValue() > 0.95))
	{
		...
		if ((fHVoltage > 0.5 * mvControlled->EnginePowerSource.MaxVoltage) ||
		    ((mvControlled->EngineType != TEngineType::ElectricSeriesMotor) && (mvControlled->EngineType != TEngineType::ElectricInductionMotor) && (true == mvControlled->Battery)))
		{
	...
	if (m_linebreakerstate == 2)
	{
		...
		if ((ggMainOnButton.SubModel == nullptr) || ((mvControlled->EngineType == TEngineType::DieselEngine) || (mvControlled->EngineType == TEngineType::DieselElectric)))
		{
			// try to finalize state change of the line breaker, set the state based on the outcome
	...
	{ // Ra: TODO: odczyty klawiatury/pulpitu nie powinny być uzależnione od istnienia modelu kabiny
		...
		{
			// nie blokujemy AI
			if ((mvOccupied->TrainType == dt_ET40) || (mvOccupied->TrainType == dt_EP05))
			{
		...
		if ((mvControlled->EngineType != TEngineType::DieselElectric) && (mvControlled->EngineType != TEngineType::ElectricInductionMotor)) // Ra 2014-09: czy taki rozdzia? ma sens?
		...
		for (int i = 0; i < 20; i++)
		{
			...
			{
				fPress[i][0] = fPress[i][1] = fPress[i][2] = 0;
		...
		if (mvControlled->ConverterFlag == true)
		{
			...
			if ((mvControlled->CompressorFlag == true) && (mvControlled->CompressorPower == 1) &&
			    ((mvControlled->EngineType == TEngineType::ElectricSeriesMotor) || (mvControlled->TrainType == dt_EZT)) &&
		...
		{ // yB - wskazniki drugiego czlonu
			...
			if (tmp)
				if (mvControlled->Battery || mvControlled->ConverterFlag)
				{
					...
					if ((mover->StLinFlag) || (mover->BrakePress > 2.0) || (mover->PipePress < 0.36))
					{
		...
		if (ggJointCtrl.SubModel != nullptr)
		{
			...
			auto const brakerangemultiplier{(mvControlled->CoupledCtrl ? mvControlled->MainCtrlPosNo + mvControlled->ScndCtrlPosNo : mvControlled->MainCtrlPosNo) /
			                                static_cast<double>(LocalBrakePosNo)};
			ggJointCtrl.UpdateValue((mvOccupied->LocalBrakePosA > 0.0 ? mvOccupied->LocalBrakePosA * LocalBrakePosNo * -1 * brakerangemultiplier :
			                                                            mvControlled->CoupledCtrl ? double(mvControlled->MainCtrlPos + mvControlled->ScndCtrlPos) : double(mvControlled->MainCtrlPos)),
			                        dsbNastawnikJazdy);
			ggJointCtrl.Update();
		}
		...
		// lights
		auto const lightpower{(InstrumentLightType == 0 ?
		                           mvControlled->Battery || mvControlled->ConverterFlag :
		                           ...
		                           InstrumentLightType == 2 ? mvControlled->ConverterFlag :
		                                                      InstrumentLightType == 3 ? mvControlled->Battery || mvControlled->ConverterFlag :
		                                                                                 InstrumentLightType == 4 ? mvControlled->Battery || mvControlled->ConverterFlag : false)};
		InstrumentLightActive = (InstrumentLightType == 3 ? true : // TODO: link the light state with the state of the master key
		                             InstrumentLightType == 4 ? (mvOccupied->iLights[end::front] != 0) || (mvOccupied->iLights[end::rear] != 0) : InstrumentLightActive);
	...
	// calculate current level of interior illumination
	{
		...
		auto const converteractive{((mvOccupied->ConverterFlag) ||
		...
		for (auto &cab : Cabine)
		{
			...
			auto const cablightlevel = ((cab.bLight == false) ? 0.f : (cab.bLightDim == true) ? 0.4f : 1.f) * (converteractive ? 1.f : 0.5f);
...
void TTrain::update_sounds(double const Deltatime)
{
	...
	if ((false == FreeFlyModeFlag) && (false == Global.CabWindowOpen) && (DynamicObject->GetVelocity() > 0.5) && (DynamicObject->IsHunting))
	{
		...
		auto const huntingamount =
		    interpolate(0.0, 1.0, clamp((mvOccupied->Vel - DynamicObject->HuntingShake.fadein_begin) / (DynamicObject->HuntingShake.fadein_end - DynamicObject->HuntingShake.fadein_begin), 0.0, 1.0));
	...
	if (mvOccupied->SecuritySystem.is_beeping())
	{
		...
		{
			dsbBuzzer.pitch(dsbBuzzer.m_frequencyoffset + dsbBuzzer.m_frequencyfactor).gain(dsbBuzzer.m_amplitudeoffset + dsbBuzzer.m_amplitudefactor).play(sound_flags::looping);
#ifdef _WIN32
	...
	if (fTachoCount >= 3.f)
	{
		...
		dsbHasler.pitch(frequency).gain(dsbHasler.m_amplitudeoffset + dsbHasler.m_amplitudefactor).play(sound_flags::exclusive | sound_flags::looping);
	}
...
void TTrain::update_sounds_radio()
{
	...
	{
		auto const volume{(true == radioenabled) && (message.first == iRadioChannel) ? 1.0 : 0.0};
		...
		do
		{
			...
			*/
			...
			else if (token == "pyscreen:")
			{
				...
				parser.getTokens(2);
				parser >> submodelname >> renderername;
				...
				const std::string rendererpath{substr_path(renderername).empty() ? // supply vehicle folder as path if none is provided
...
{ // zwraca współrzędne widoku kamery z lusterka
	...
	{
	case 2: // tylna (-1)
		return DynamicObject->mMatrix * Math3D::vector3(mvOccupied->Dim.W * (lewe ? -0.5 : 0.5) + 0.2 * (lewe ? -1 : 1), 1.5 + Cabine[iCabn].CabPos1.y, Cabine[iCabn].CabPos1.z);
	case 1: // przednia (1)
	default:
		return DynamicObject->mMatrix * Math3D::vector3(mvOccupied->Dim.W * (lewe ? 0.5 : -0.5) + 0.2 * (lewe ? 1 : -1), 1.5 + Cabine[iCabn].CabPos1.y, Cabine[iCabn].CabPos2.z);
	}
...
{ // taka proteza: chcę podłączyć
	...
	if ((d->MoverParameters->TrainType == dt_EZT) || (d->MoverParameters->TrainType == dt_DMU))
	{

		if ((d->NextConnected() != nullptr) && (true == TestFlag(d->MoverParameters->Couplers[end::rear].AllowedFlag, coupling::permanent)))
		{
		...
		else if ((d->PrevConnected() != nullptr) && (true == TestFlag(d->MoverParameters->Couplers[end::front].AllowedFlag, coupling::permanent)))
		{
...
void TTrain::MoveToVehicle(TDynamicObject *target)
{
	...
	else
	{
		...
		InitializeCab(Occupied()->CabNo, Dynamic()->asBaseDir + Occupied()->TypeName + ".mmd");
...
Math3D::vector3 TTrain::clamp_inside(Math3D::vector3 const &Point) const
{
	...
	return {clamp(Point.x, Cabine[iCabn].CabPos1.x, Cabine[iCabn].CabPos2.x), clamp(Point.y, Cabine[iCabn].CabPos1.y + 0.5, Cabine[iCabn].CabPos2.y + 1.8),
...
void TTrain::radio_message(sound_source *Message, int const Channel)
{
	...
	auto const radioenabled{(true == mvOccupied->Radio) && (mvControlled->Battery || mvControlled->ConverterFlag)};
	auto const volume{(true == radioenabled) && (Channel == iRadioChannel) ? 1.0 : 0.0};
...
void TTrain::set_cab_controls(int const Cab)
{
	...
	if (mvOccupied->PantSwitchType != "impulse")
	{
		...
		{
			ggPantFrontButton.PutValue((mvControlled->PantFrontUp ? 1.f : 0.f));
		}
		if (ggPantFrontButtonOff.SubModel)
		{
			ggPantFrontButtonOff.PutValue((mvControlled->PantFrontUp ? 0.f : 1.f));
		}
		// NOTE: currently we animate the selectable pantograph control for both pantographs
		// TODO: implement actual selection control, and refactor handling this control setup in a separate method
		if (ggPantSelectedButton.SubModel)
		{
			ggPantSelectedButton.PutValue((mvControlled->PantFrontUp ? 1.f : 0.f));
		}
		if (ggPantSelectedDownButton.SubModel)
		{
			ggPantSelectedDownButton.PutValue((mvControlled->PantFrontUp ? 0.f : 1.f));
		}
	}
	if (mvOccupied->PantSwitchType != "impulse")
	{
		...
		{
			ggPantRearButton.PutValue((mvControlled->PantRearUp ? 1.f : 0.f));
		}
		if (ggPantRearButtonOff.SubModel)
		{
			ggPantRearButtonOff.PutValue((mvControlled->PantRearUp ? 0.f : 1.f));
		}
		// NOTE: currently we animate the selectable pantograph control for both pantographs
		// TODO: implement actual selection control, and refactor handling this control setup in a separate method
		if (ggPantSelectedButton.SubModel)
		{
			ggPantSelectedButton.PutValue((mvControlled->PantRearUp ? 1.f : 0.f));
		}
		if (ggPantSelectedDownButton.SubModel)
		{
			ggPantSelectedDownButton.PutValue((mvControlled->PantRearUp ? 0.f : 1.f));
		}
	}
	...
	ggPantCompressorValve.PutValue(mvControlled->bPantKurek3 ? 0.f : // default setting is pantographs connected with primary tank
	                                   1.f);
	ggPantCompressorButton.PutValue(mvControlled->PantCompFlag ? 1.f : 0.f);
	// converter
	...
	ggMaxCurrentCtrl.PutValue((true == mvControlled->ShuntModeAllow ? (true == mvControlled->ShuntMode ? 1.f : 0.f) : (mvControlled->Imax == mvControlled->ImaxHi ? 1.f : 0.f)));
	// lights
	ggLightsButton.PutValue(mvOccupied->LightsPos - 1);

	int const vehicleside = (mvOccupied->ActiveCab == 1 ? end::front : end::rear);
	...
	{
		ggMotorBlowersAllOffButton.PutValue((mvControlled->MotorBlowers[end::front].is_disabled || mvControlled->MotorBlowers[end::front].is_disabled) ? 1.f : 0.f);
	}
	// tempomat
...
bool TTrain::initialize_button(cParser &Parser, std::string const &Label, int const Cabindex)
{
	...
	std::unordered_map<std::string, bool *> const autolights = {{"i-doorpermit_left:", &mvOccupied->Doors.instances[side::left].open_permit},
...
bool TTrain::initialize_gauge(cParser &Parser, std::string const &Label, int const Cabindex)
{
	...
	std::unordered_map<std::string, bool *> const autoboolgauges = {{"doorstep_sw:", &mvOccupied->Doors.step_enabled}, {"coolingfans_sw:", &mvControlled->RVentForceOn}};
	{
...
void train_table::update(double dt)
{
	for (TTrain *train : m_items)
	{
		...
		if (simulation::Train != train && Global.network_servers.empty() && !Global.network_client)
		{
			purge(train->Dynamic()->name());
		}
	}
